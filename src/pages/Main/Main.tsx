...
export const Main = () => {
  ...
  return (
    <div className="overflow-y-auto">
      <header className="flex justify-center h-16 items-center border-b-2 border-b-[#FFCC15] pb-3 rounded-[30px] mt-3">
        <h2 className="text-white text-center font-inter text-[25px] font-bold tracking-[1px] flex items-center gap-2">
          {doughroom?.title}
        </h2>
          ...
          <Accordion type="single" collapsible className="my-4 space-y-4">
            {retsepts.map((retsept) => (
              <AccordionItem key={retsept._id} value={retsept.title}>
                <AccordionTrigger className="hover:no-underline bg-white text-[#1C2C57] p-2 rounded-lg border-2 border-yellow-400">
                  {retsept.title}
                </AccordionTrigger>
                <AccordionContent>
